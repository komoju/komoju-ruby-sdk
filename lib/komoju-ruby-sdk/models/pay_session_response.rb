...
module Komoju
  class PaySessionResponse
    ...
    # URL for the payment app, if applicable.
    ...
    def self.openapi_types
      {
        ...
        :'payment' => :'Payment',
        :'app_url' => :'String',
        :'customer' => :'PaySessionResponseCustomer'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Komoju::PaySessionResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::PaySessionResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [redirect_url, status, payment, app_url, customer].hash
    end
