...
module Komoju
  # 
  class PaymentDetailsPayPay < ApiModelBase
    # Payment method type.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Komoju::PaymentDetailsPayPay` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::PaymentDetailsPayPay`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'user_authorization_id')
        self.user_authorization_id = attributes[:'user_authorization_id']
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["paypay"])
