...
module Komoju
  # 
  class ResponsePaymentDetailsCreditCard
    # Payment method type.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::ResponsePaymentDetailsCreditCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @number.to_s.length < 8
        invalid_properties.push('invalid value for "number", the character length must be great than or equal to 8.')
      end
    ...
    def number=(number)
      ...
      if number.to_s.length < 8
        fail ArgumentError, 'invalid value for "number", the character length must be great than or equal to 8.'
      end
