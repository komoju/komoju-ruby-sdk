...
module Komoju
  # 
  class CreateSessionRequestWithCustomerMode < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Komoju::CreateSessionRequestWithCustomerMode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::CreateSessionRequestWithCustomerMode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      mode_validator = EnumAttributeValidator.new('String', ["customer"])
    ...
    def hash
      [mode, return_url, amount, currency, email, expires_in_seconds, external_customer_id, payment_types, default_locale, line_items, metadata, customer_id].hash
    end
