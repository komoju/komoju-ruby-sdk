...
module Komoju
  # 
  class PaymentDetailsPix < ApiModelBase
    # Payment method type.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Komoju::PaymentDetailsPix` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::PaymentDetailsPix`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @cpf.nil?
        invalid_properties.push('invalid value for "cpf", cpf cannot be nil.')
      end

      pattern = Regexp.new(/([0-9]{3}[.]?[0-9]{3}[.]?[0-9]{3}[-]?[0-9]{2})/)
      if @cpf !~ pattern
        invalid_properties.push("invalid value for \"cpf\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["pix"])
      return false unless type_validator.valid?(@type)
      return false if @name.nil?
      return false if @name.to_s.length > 255
      return false if @email.nil?
      return false if @cpf.nil?
      return false if @cpf !~ Regexp.new(/([0-9]{3}[.]?[0-9]{3}[.]?[0-9]{3}[-]?[0-9]{2})/)
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["pix"])
    ...
    def cpf=(cpf)
      if cpf.nil?
        fail ArgumentError, 'cpf cannot be nil'
      end

      pattern = Regexp.new(/([0-9]{3}[.]?[0-9]{3}[.]?[0-9]{3}[-]?[0-9]{2})/)
      if cpf !~ pattern
        fail ArgumentError, "invalid value for \"cpf\", must conform to the pattern #{pattern}."
      end
