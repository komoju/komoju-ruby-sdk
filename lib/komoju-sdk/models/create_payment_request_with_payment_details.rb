...
module Komoju
  # 
  class CreatePaymentRequestWithPaymentDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Komoju::CreatePaymentRequestWithPaymentDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Komoju::CreatePaymentRequestWithPaymentDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [amount, capture, description, tax, currency, external_order_num, return_url, cancel_url, locale, metadata, mcc, statement_descriptor, fraud_details, platform_details, payment_details].hash
    end
