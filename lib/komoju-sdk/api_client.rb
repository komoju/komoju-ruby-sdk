...
module Komoju
  class ApiClient
    ...
    def initialize(config = Configuration.default)
      @config = config
      @user_agent = "komoju-ruby-sdk/#{VERSION} api/2025-01-28 ruby/#{RUBY_VERSION}"
      @default_headers = {
    ...
    def convert_to_type(data, return_type)
      ...
      when 'Time'
        ...
        Time.parse(data) rescue data
      when 'Date'
        # parse date time (expecting ISO 8601 format)
        Date.parse(data) rescue data
      when 'Object'
      ...
      when /\AArray<(.+)>\z/
        # e.g. Array<Pet>
        sub_type = $1
        data.map { |item| convert_to_type(item, sub_type) rescue item }
      when /\AHash\<String, (.+)\>\z/
        # e.g. Hash<String, Integer>
        sub_type = $1
        {}.tap do |hash|
          data.each { |k, v| hash[k] = convert_to_type(v, sub_type) rescue v }
        end
      else
        # models (e.g. Pet) or oneOf
        klass = Komoju.const_get(return_type)
        begin
